...
export const ParticipantsDashboard: React.FC = () => {
  ...
  return (
    <div className="space-y-6 text-right relative" dir="rtl">
      ...
      {/* Intro Header Card with ambient fluid blur */}
      <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-slate-900/60 backdrop-blur-xl p-6 md:p-8 shadow-2xl">
        <div className="absolute top-0 left-0 -ml-16 -mt-16 h-48 w-48 rounded-full bg-pink-500/[0.08] blur-3xl"></div>
        <div className="absolute bottom-0 right-0 -mr-16 -mb-16 h-48 w-48 rounded-full bg-indigo-500/[0.08] blur-3xl"></div>

        <div className="relative z-10 flex flex-col lg:flex-row items-start lg:items-center justify-between gap-6">
          <div className="space-y-3">
            <div className="flex items-center gap-2">
              <span className="flex h-2.5 w-2.5 rounded-full bg-pink-500 animate-pulse"></span>
              <span className="text-xs font-black text-pink-400 uppercase tracking-wider bg-pink-500/10 px-2.5 py-1 rounded-lg">
                رتبه‌بندی کارشناسی و پیش‌بینی‌ها
              </span>
            </div>
            <h2 className="text-2xl md:text-3xl font-black text-slate-100 font-sans tracking-tight">
              جدول سراسری انتشار شرکت‌کنندگان و پیش‌بینی‌ها
            </h2>
            <p className="text-slate-400 text-xs md:text-sm max-w-xl leading-relaxed">
              ثبت، پایش و انتشار رکوردهای شرکت‌کنندگان و کاربران فعال اپلیکیشن به صورت آنلاین. رتبه‌ها و محاسبات کارشناسی به صورت کاملاً زنده اعمال و مدیریت می‌شود.
            </p>
          </div>
            ...
            {isAdminMode ? (
              <button
                ...
                onClick={() => {
                  ...
                  triggerToast("وضعیت کاربری مدیر لغو و نمایش عمومی فعال شد", "info");
                }}
                className="flex items-center gap-2 px-5 py-2.5 rounded-xl bg-amber-500/10 border border-amber-500/30 text-amber-300 font-bold text-xs hover:bg-amber-500/20 duration-150 cursor-pointer shadow-lg shadow-amber-500/[0.02]"
              >
                <Unlock size={14} className="text-amber-400" />
                <span>خروج از پنل مدیریت ورودی‌ها</span>
              </button>
            ) : (
              <button
                id="open-admin-dialog-btn"
                onClick={() => {
                  setAdminPasswordInput("");
                  setAdminError(null);
                  setIsAdminModalOpen(true);
                }}
                className="flex items-center gap-2 px-6 py-3 rounded-xl bg-white/[0.03] hover:bg-white/[0.07] border border-white/10 text-slate-200 hover:text-pink-400 font-black text-xs duration-150 cursor-pointer shadow-xl backdrop-blur-md"
              >
                <Settings size={14} className="animate-spin duration-3000 text-pink-400" />
                <span>ورود کاربری مدیر (مدیریت داده‌های ورودی)</span>
              </button>
            )}
          </div>
        </div>
      ...
      {/* 🚀 BREATHTAKING MOTIVATIONAL CAMPAIGN CARD (افزایش جذابیت و هیجان مشارکت) */}
      <div id="motivation-football-hero" className="relative overflow-hidden rounded-3xl border border-pink-500/20 bg-gradient-to-r from-purple-950/20 via-slate-900/90 to-pink-950/20 p-6 md:p-8 shadow-2xl transition-all hover:border-pink-500/30">
        {/* Glow Spheres */}
        <div className="absolute top-0 right-0 w-44 h-44 bg-pink-500/[0.07] rounded-full blur-3xl pointer-events-none animate-pulse"></div>
        <div className="absolute bottom-0 left-0 w-44 h-44 bg-indigo-500/[0.07] rounded-full blur-3xl pointer-events-none animate-pulse"></div>

        <div className="relative z-10 grid grid-cols-1 md:grid-cols-12 gap-6 items-center">
          
          {/* Main Slogan area */}
          <div className="md:col-span-7 space-y-4 text-right">
            <div className="inline-flex items-center gap-1.5 px-3 py-1 bg-gradient-to-r from-pink-500/15 to-purple-500/15 border border-pink-500/20 text-[10px] md:text-xs font-black text-pink-300 rounded-full select-none animate-bounce">
              <Trophy size={11} className="text-yellow-400 rotate-12" />
              <span>کل‌کل داغ جوانان فوتبالی و کارشناسان برتر!</span>
            </div>

            <h3 className="text-xl md:text-2xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white via-pink-100 to-indigo-200 tracking-tight leading-snug">
              آیا می‌توانید پیش‌گوی اعظم نهایی جام لقب بگیرید؟ 🔮🏆
            </h3>

            <p className="text-slate-350 text-xs md:text-sm leading-relaxed Persian-font font-medium">
              محیط فانتزی و زبا مسابقات جهانی نوجوانان را پیش‌بینی کرده، مدال‌های نفیس فوتبالی کسب کنید و نام خود را به صدر جدول بکشید! تاکنون <strong className="text-pink-400 font-mono font-black">{totalCount} تحلیل‌گر برتر</strong> با ثبت برگه مستقل، هوش تاکتیکی خود را به رخ کشیده‌اند.
            </p>

            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2 text-[11px] sm:text-xs font-bold text-slate-300">
              <div className="flex items-center gap-2">
                <span className="text-pink-400 select-none text-base">⭐</span>
                <span>کسب نشان طلایی "پیش‌گوی اعظم" با فتح امتیاز کامل</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-purple-400 select-none text-base">🔮</span>
                <span>پیش‌بینی مستقل تمام مسیرهای پلی‌آف تا فینال</span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-cyan-400 select-none text-base">📈</span>
                <span>میانگین امتیازات رقبا هم‌اکنون: <strong className="text-cyan-400 font-mono font-extrabold">{averageScore} pts</strong></span>
              </div>
              <div className="flex items-center gap-2">
                <span className="text-emerald-400 select-none text-base">⚡</span>
                <span>بروزرسانی فوق‌العاده زنده و آنی رنکینگ‌ها</span>
              </div>
            </div>
          </div>

          {/* Visual Trophy Display or Actions */}
          <div className="md:col-span-5 flex flex-col items-center justify-center gap-4 bg-white/[0.02] border border-white/5 rounded-2xl p-5 relative">
            <div className="absolute top-0 right-0 -mr-2 -mt-2 bg-pink-500 text-white font-black text-[9px] px-2 py-0.5 rounded-full select-none animate-pulse">
              رایگان و فوری!
            </div>
            
            <div className="relative flex items-center justify-center">
              <div className="absolute w-20 h-20 bg-purple-500/10 rounded-full blur-xl animate-pulse"></div>
              <span className="text-5xl animate-bounce duration-3000 filter drop-shadow-[0_4px_12px_rgba(234,179,8,0.4)]">🏆</span>
            </div>
            
            <div className="text-center space-y-1">
              <h4 className="text-xs font-black text-slate-100 Persian-font">پیش‌بینی زودهنگام قهرمان نهایی طلایی 🏅</h4>
              <p className="text-[10px] text-slate-400">یک کشور را برگزینید تا ۱۵۰ امتیاز بونس به شما تخصیص یابد.</p>
            </div>
            ...
            <AnimatePresence mode="wait">
              {!campaignChamp ? (
                <motion.button
                  ...
                  onClick={() => {
                    setIsChampSelectorOpen(true);
                    setChampSearchQuery("");
                  }}
                  className="w-full py-3 rounded-xl bg-gradient-to-r from-pink-500 to-purple-600 hover:from-pink-600 hover:to-purple-700 text-white font-black text-xs duration-150 cursor-pointer shadow-lg shadow-purple-500/25 active:scale-95 text-center flex items-center justify-center gap-2 outline-none border border-white/5"
                >
                  <Trophy size={14} className="text-yellow-400 animate-pulse" />
                  <span>انتخاب قهرمان جام و دریافت امتیاز هدیه!</span>
                </motion.button>
              ) : (
                <motion.div
                  key="chosen-team"
                  ...
                  animate={{ opacity: 1, scale: 1 }}
                  exit={{ opacity: 0, scale: 0.95 }}
                  className="w-full bg-slate-950/60 border border-indigo-500/30 p-3.5 rounded-xl text-center space-y-3.5 relative overflow-hidden"
                >
                  <div className="absolute top-0 left-0 right-0 h-1 bg-gradient-to-r from-pink-500 via-purple-500 to-indigo-500"></div>
                  <p className="text-[10px] text-slate-300 font-bold">پیش‌گویی شما با موفقیت ثبت شد! 🔥</p>
                  
                  <div className="flex items-center justify-center gap-2 bg-slate-900/80 py-1.5 px-3 rounded-xl border border-white/5 w-fit mx-auto shadow-inner">
                    <span className="text-2xl select-none">{TEAMS[campaignChamp]?.flag || "🏳️"}</span>
                    <span className="text-xs font-black text-pink-400">{TEAMS[campaignChamp]?.name || campaignChamp}</span>
                  </div>

                  <p className="text-[9px] text-emerald-400 font-black tracking-tight animate-pulse">
                    ⚡ ۱۵۰+ امتیاز هدیه ویژه کارشناسی برای شما افزوده شد!
                  </p>
                  ...
                  <button
                    ...
                    className="block mx-auto text-[9px] text-slate-400 hover:text-white underline cursor-pointer hover:scale-105 duration-100 font-bold"
                  >
                    تغییر تیم قهرمان نهایی
                  </button>
                </motion.div>
              )}
            </AnimatePresence>
      ...
      {/* 📊 Unified Summary Card (کارت خلاصه وضعیت پیش‌بینی‌ها) */}
      <div id="wc-predictor-summary-card" className="relative overflow-hidden rounded-3xl border border-white/10 bg-gradient-to-br from-slate-900/80 via-slate-900/60 to-indigo-950/40 backdrop-blur-xl p-6 md:p-8 shadow-2xl">
        <div className="absolute top-0 right-0 w-36 h-36 bg-pink-500/[0.05] rounded-full blur-3xl pointer-events-none"></div>
        <div className="absolute bottom-0 left-0 w-36 h-36 bg-indigo-500/[0.08] rounded-full blur-3xl pointer-events-none"></div>
        
        <div className="relative z-10 flex flex-col lg:flex-row items-center justify-between gap-6">
          <div className="space-y-2 text-right">
            <div className="flex items-center gap-2">
              <span className="p-1 px-2.5 rounded-md bg-indigo-500/10 text-indigo-400 text-[10px] font-black border border-indigo-500/20">گزارش تجمعی</span>
              <p className="text-slate-400 text-xs font-semibold">بررسی خلاصه مشارکت و عملکرد پیش‌بینی کاربران فعال</p>
            </div>
            <h3 className="text-lg md:text-xl font-black text-slate-100 flex items-center gap-2">
              ...
              <span>کارت گزارش عملکرد کل پیش‌بینی‌های سامانه</span>
            </h3>
          </div>

          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 w-full lg:w-auto">
            
            {/* Stat Item: Total Participants */}
            <div className="flex items-center gap-3 bg-white/[0.01] border border-white/5 rounded-2xl p-4 sm:px-6 hover:bg-white/[0.03] duration-150 transition-all">
              <div className="p-3 bg-pink-500/10 text-pink-400 rounded-xl border border-pink-500/10 shadow-lg">
                <Users size={22} />
              </div>
              <div className="text-right">
                <span className="block text-[10px] text-slate-500 font-bold mb-0.5">کل شرکت‌کنندگان</span>
                <div className="flex items-baseline gap-1 justify-start">
                  <span className="text-xl font-mono font-black text-slate-100">{totalCount}</span>
                  <span className="text-[10px] text-slate-400 font-bold">نفر</span>
                </div>
              </div>
            </div>

            {/* Stat Item: Average Prediction Score */}
            <div className="flex items-center gap-3 bg-white/[0.01] border border-white/5 rounded-2xl p-4 sm:px-6 hover:bg-white/[0.03] duration-150 transition-all">
              <div className="p-3 bg-teal-500/10 text-teal-400 rounded-xl border border-teal-500/10 shadow-lg animate-pulse">
                <Trophy size={22} />
              </div>
              <div className="text-right">
                <span className="block text-[10px] text-slate-500 font-bold mb-0.5">میانگین امتیاز پیش‌بینی‌ها</span>
                <div className="flex items-baseline gap-1 justify-start">
                  <span className="text-xl font-mono font-black text-teal-400">{averageScore}</span>
                  <span className="text-[10px] text-teal-450 font-bold text-teal-400">امتیاز</span>
                </div>
              </div>
            </div>
