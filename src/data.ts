...
/** First half + halftime break + second half + stoppage buffer (minutes). */
const MATCH_FIRST_HALF_MIN = 45;
const MATCH_HALFTIME_BREAK_MIN = 15;
const MATCH_SECOND_HALF_MIN = 45;
const MATCH_STOPPAGE_BUFFER_MIN = 15;

export type MatchTimePhase = "scheduled" | "live" | "halftime" | "finished";

export function getMatchEndTime(kickoff: Date): Date {
  const totalMin =
    MATCH_FIRST_HALF_MIN +
    MATCH_HALFTIME_BREAK_MIN +
    MATCH_SECOND_HALF_MIN +
    MATCH_STOPPAGE_BUFFER_MIN;
  return new Date(kickoff.getTime() + totalMin * 60 * 1000);
}

export function getMatchTimePhase(
  kickoff: Date,
  now: Date = new Date()
): { phase: MatchTimePhase; minute: number | null } {
  if (now < kickoff) {
    return { phase: "scheduled", minute: null };
  }

  const end = getMatchEndTime(kickoff);
  if (now >= end) {
  ...
  if (elapsedMin <= MATCH_FIRST_HALF_MIN) {
    return { phase: "live", minute: Math.max(1, elapsedMin) };
  }

  if (elapsedMin <= MATCH_FIRST_HALF_MIN + MATCH_HALFTIME_BREAK_MIN) {
    return { phase: "halftime", minute: 45 };
  }

  const secondHalfMinute =
    elapsedMin - (MATCH_FIRST_HALF_MIN + MATCH_HALFTIME_BREAK_MIN) + 46;
  return {
    phase: "live",
    minute: Math.min(90 + MATCH_STOPPAGE_BUFFER_MIN, secondHalfMinute),
  };
}

export interface MatchDisplayStatus {
  phase: MatchTimePhase;
  minute: number | null;
  isLocked: boolean;
...
export function getMatchDisplayStatus(
  matchId: string,
  match: { isOfficial?: boolean; isLive?: boolean; minute?: number },
  now: Date = new Date()
): MatchDisplayStatus {
  const kickoff = getMatchKickoffDate(matchId);
  const { phase, minute: computedMinute } = getMatchTimePhase(kickoff, now);

  const showOfficial = !!match.isOfficial && phase !== "scheduled";
  const showHalftime = !showOfficial && phase === "halftime";
  const showLive = !showOfficial && phase === "live";
  const showFinished = !showOfficial && phase === "finished";

  let minute: number | null = null;
  if (showLive || showHalftime) {
    minute =
      match.isLive && match.minute != null && match.minute > 0
        ? match.minute
        : computedMinute;
  }

  const isLocked = phase !== "scheduled" || showOfficial;
...
/** Strip stale live/official flags when kickoff hasn't arrived or match window ended. */
export function reconcileMatchWithTime(match: Match, now: Date = new Date()): Match {
  const kickoff = getMatchKickoffDate(match.id);
  const { phase } = getMatchTimePhase(kickoff, now);

  if (phase === "scheduled") {
    if (match.isLive || match.isOfficial) {
      return { ...match, isLive: false, isOfficial: false, minute: undefined };
    }
    return match;
  }

  if (phase === "finished" && match.isLive) {
    return { ...match, isLive: false, minute: undefined };
  }
